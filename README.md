...
**Осторожно при работе с текстурами**: Проект собирается на Jenkins через Godot Headless. Не коммитьте сгенерированные вручную файлы `.import` для новых текстур, так как они содержат локальные пути, что приведет к ошибкам на CI. Оставьте `.import` для генерации серверу.
