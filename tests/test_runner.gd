...
func _ready() -> void:
    ...
    # Elevator door alignment used to be re-checked here too, duplicating tests/test_elevator_
    # alignment.gd (its own Jenkins stage) with a second copy of the same bounds math - removed
    # 2026-08-24 when the door was reworked from one sliding panel to two (sliding_door_pair.gd,
    # see elevator_door.tscn) rather than updating both copies in lockstep. That dedicated test
    # now does real pass/fail assertions (this copy never did - it only ever printed an ASCII
    # diagram and unconditionally quit(0)), so nothing here duplicated it in coverage, only in
    # maintenance risk.
    print("\n✅ [OK] All autotests passed.")
    get_tree().quit(0)
