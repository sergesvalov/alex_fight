...
def process_file():
    ...
    out.append('transform = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, 0, 0, -32.5)')
    out.append('use_collision = true')
    out.append('collision_layer = 2')
    out.append('size = Vector3(7, 0.5, 85)') # from +10 to -75
    ...
    out.append('transform = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, 0, 4.25, -32.5)')
    out.append('use_collision = true')
    out.append('collision_layer = 2')
    out.append('size = Vector3(7, 0.5, 85)')
    out.append('material = SubResource("StandardMaterial3D_floor")')
    out.append('')
    ...
    length = prev_z - (-60.0)
    center = (prev_z + -60.0) / 2.0
    if length > 0:
        add_wall(f"CorrWallW7", -3.5, 2, center, length)
    ...
    center = (prev_z + -60.0) / 2.0
    if length > 0:
        add_wall(f"CorrWallE10", 3.5, 2, center, length)
    ...
    for i, (c, lbl) in enumerate(zip(L_centers, dbl_labels)):
        out.append(f'[node name="RoomLabel_{lbl}" type="Label3D" parent="NavigationRegion3D/HotelGeometry"]')
        out.append(f'transform = Transform3D(0, 0, -1, 0, 1, 0, 1, 0, 0, -3.49, 2.2, {c+0.5})')
        out.append(f'text = "{lbl}"')
        out.append('font_size = 72')
        out.append('outline_size = 12')
        out.append('modulate = Color(0.8, 0.7, 0.2, 1)')
        out.append('')

    for i, (c, lbl) in enumerate(zip(R_centers, sngl_labels)):
        out.append(f'[node name="RoomLabel_{lbl}" type="Label3D" parent="NavigationRegion3D/HotelGeometry"]')
        out.append(f'transform = Transform3D(0, 0, 1, 0, 1, 0, -1, 0, 0, 3.49, 2.2, {c+0.5})')
        out.append(f'text = "{lbl}"')
        out.append('font_size = 72')
    ...
    with open('scenes/levels/hotel_siberia/base_hotel_level.tscn', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Rewritten base_hotel_level.tscn successfully.")
