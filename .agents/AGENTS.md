...
- **Stairs are gated, not free (added 2026-08-22)**: because both stair blocks physically connect floor N's stairwell straight into floor N+1's corridor (see above), an `Area3D` (`stairs_gate.gd`) sits in every stairwell doorway - South Stairs' `SouthStairsDoor`, North Stairs' `DoorEast`/`DoorWest` - one per floor, tagged with that floor's number. Crossing a gate whose floor number differs from `GameStateManager.current_floor` bounces the player straight back (Y-shift by one floor-to-floor height) unless `GameStateManager.floor_stairs_unlocked` is true, which `collect_tape()` sets once that floor's 3 tapes are collected. The elevator is intentionally exempt - it's the always-open way to change floors. See MECHANICS.md section 11.
