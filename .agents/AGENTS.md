...
|---------|----------|----------|------------|
| WallEast | +4.65..+4.85 | -30.0..-25.1 | Восточная стена блока |
| WallWest | -2.75..-2.55 | -30.0..-25.1 | Западная стена блока |
| WallNorth | -2.75..+4.85 | -30.0..-29.8 | Северная (глухая) стена |
| WallSouth | -2.75..+4.85 | -25.2..-25.1 | Южная стена, граница коридора |
| DoorHoleEast | +3.15..+4.45 | -25.2..-25.1 | Нижний вход (Y=0..2.2) |
| DoorHoleWest | -2.45..-1.15 | -25.2..-25.1 | Верхний выход (Y=4.5..6.7) |
| CenterDivider | +0.95..+1.15 | -29.0..-25.1 | Перегородка между маршами |
| MidLanding | -2.65..+4.75 | -30.0..-28.5 | Площадка Y=2.25, полная ширина |

### Вид сверху — план (масштаб: 1 символ ≈ 0.5м)

```
         local X:  -3.7  -3.0  -2.0  -1.0   0.0  +1.0  +2.0  +3.0  +3.7
  local Z=0.0  ████████████████████████████████████████████████████████  ← WallNorth (global Z=-30.0)
               ██                                   ║                 ██
  Z=0.5        ██   NW Landing / MidLanding area    ║  NE Landing     ██
  Z=1.0        ██   Y=2.25 (flat)                   ║  Y=2.25 (flat)  ██
  Z=1.5        ██                                   ║                 ██
               ██     W E S T  S H A F T            ║  E A S T  S H A F T
  Z=2.0        ██   WestFlight (Y↑ south→north)     ║  EastFlight (Y↑ south→north)
  Z=2.5        ██   Y: 4.5 (south) → 2.25 (north)  ║  Y: 0 (south) → 2.25 (north)
  Z=3.0        ██                                   ║                 ██
  Z=3.5        ██                                   ║                 ██
  Z=4.0        ██                                   ║                 ██
  Z=4.5        ██                                   ║                 ██
  Z=4.9  ████████ ████ ████████████████████████████ ████ ██ ████ ██ ████  ← WallSouth (global Z=-25.1)
                   ▲               ▲                 ▲       ▲
              DoorWest          пустота           пустота  DoorEast
              (Y=4.5..6.7)    (solid wall)               (Y=0..2.2)
              upper exit                                  lower entry
         global X: -2.65      +0.95                     +3.15  +4.45  +4.75
```

### Вид сбоку — профиль по оси Z (центральная линия блока)
...
4.5 |      X=-2.8, Y=4.5       ║            ║   |
    ╔══════════╗               ║            ║   |
    ║ TopLand  ║               ║            ║   |
4.0 ║ Y=4.0←  ╚══════════════╝ WestFlight  ║   |
    ║  WestFlight rises here                ║   |
2.25╚═══════════╗ MidLanding ╔══════════════╝   |
    |            ╚═══════════╝                   |
    |                              ╔═════════════╝
2.2 |                              ║ EastFlight top
    |                              ║
0.0 ╚══════════════════════════════╝  ← Entry (East door, Y=0)
    N (Z=0, global-30)          S (Z=4.9, global-25.1)
    [WallNorth]            [WallSouth + doors]
```

### Правила для AI-агентов

> [!IMPORTANT]
> **Высоты лестницы**: Блок поднимает игрока с Y=0 (вход восток) до Y=4.5 (выход запад). Это равно `y_step = corridor_height + floor_thickness = 4.0 + 0.5 = 4.5`.

> [!NOTE]
> **Рампы**: `EastFlight` и `WestFlight` — `CSGPolygon3D` с поворотом -90° по Y. Transform: `(0,0,-1, 0,1,0, 1,0,0, ox,0,4.5)`. Полигон `(0,Y_low, 3.0,Y_high, 3.0,Y_high-0.2, 0,Y_low-0.2)` кодирует наклон: `polygon.x→world -Z`, `polygon.y→world Y`, `depth→world +X`.

> [!NOTE]
> **MidLanding**: плоская площадка Y=2.25 у северной стены (local Z=0..1.5), перекрывает всю ширину блока. Соединяет оба пролёта физически.

> [!WARNING]
> **Вырез в глобальном полу**: Генератор вырезает дыру шириной 7.2м в зоне X=[-2.55..+4.65], Z=[-30..-25.2]. Блок `north_stairs.tscn` должен точно закрывать эту дыру своими стенами и рампами. Смещение `inst.position.X=1.05` центрирует блок (local X=[-3.6..+3.6]) в дыре (global X=[-2.55..+4.65]).
