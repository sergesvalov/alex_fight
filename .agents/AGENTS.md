...
> [!IMPORTANT]
> **Logs let you stop guessing blindly - add them before proposing a fix, not after.** This project's whole recurring pattern this session (robot rotation spike, robot never attacking, elevator door-hole size, a stale-build terminal regression) was only ever actually solved by adding a `print()` at the exact point of doubt and reading a real log - never by reasoning from the `.tscn`/`.gd` source alone, which describes intent, not runtime reality (a stale exported build, an unbaked navmesh, a variable set before physics settles - none of that is visible from the source). When the user reports something that "shouldn't be possible" given the code as written, the fastest correct move is almost always: add a targeted print/diagnostic right where the disagreement between code-intent and observed-behavior would show up, ask for a fresh log, and read real numbers - not to re-derive a theory from the source a second time.
...
Floor 3 (not floor 4 - corrected 2026-08-23 per user report; floor 4 is the starting floor and must stay fully open, no corridor gating at all) has its own permanent corridor-splitting barrier (`corridor_barrier.gd`, only ever created there, `_add_floor3_corridor_barrier()`) that keeps the elevator/North Stairs unreachable until floor 3's own 3 tapes are collected - completing them switches the barrier off for good AND immediately unlocks floor 5. The secret exit door (always floor 4 -> floor 3) and this barrier's removal are two INDEPENDENT events inside the same `_on_all_tapes_collected()` handler now, each with its own one-time guard (`secret_portal_active` for the door, `floor3_corridor_unlocked` + `current_floor == 3` for the barrier) - they used to share one guard back when both lived on floor 4, which is no longer true. Deliberately pairing two different "everyday nightmare" tropes: a door leading who-knows-where (floor 4, the moment you first complete tapes), and a corridor that never actually gets you to the end no matter how far you walk (floor 3, the floor that door leads to). See MECHANICS.md sections 10, 11, 13 and 14.
...
## Elevator Shaft Territory Map (шахта лифта + 2м во все стороны)

Мировые координаты (f_scale=1, не зависят от этажа — все этажи используют один и тот же X/Z, отличается только Y). Верифицировано по `hotel_level_generator.gd` (`ELEVATOR_CENTER_X/Z`, `CORRIDOR_EAST_EDGE_X`, `_generate_maintenance_room()`) и `elevator_shaft.tscn`, и сверено с реальными `global_pos` из `[ElevatorShaft]`-диагностики (`elevator_controller.gd::_log_shaft_geometry()`).

### Габариты шахты лифта (без учёта зеркалирования `scale.z=-1.0` — числа уже в мировых координатах)

| Элемент | Global X | Global Z | Примечание |
|---------|----------|----------|------------|
| Кабина (интерьер) | 4.95..9.45 | -30.0..-25.0 | `ElevatorFloor`/потолок 4.5×5.0м |
| West Wall | 4.85..5.05 | -30.0..-25.0 | = `CORRIDOR_EAST_EDGE_X` (граница с коридором/сев. лестницей) |
| East Wall | 9.35..9.55 | -30.0..-25.0 | вплотную к стене техпомещения (9.65) |
| South Wall | 4.85..9.55 | -30.0..-29.8 | вплотную к внешней стене здания (`BUILDING_LENGTH_Z` начинается с Z=-30) |
| North Wall (с проёмом) | 4.85..9.55 | -25.2..-25.0 | `ElevatorDoorHole` вырезает X=6.6..7.8 |
| Дверной проём (в North Wall) | 6.6..7.8 | -25.2..-25.0 | `elevator_door.tscn` — две панели по 0.65м |
| Рамка проёма (`ElevatorFrame*`) | 6.52..7.88 (Top/Bottom), 4.85/9.55±0.08 (Left/Right) | ~-25.24..-24.96 | тонкие полосы у самого North Wall, не влияют на проходимость |

### Соседи в радиусе 2м

| Направление | Что там | Global X / Z |
|---|---|---|
| Запад (X<4.85) | Северная лестница (см. отдельную карту выше), восточная стена лестницы X=4.65..4.85 | X: 2.85..4.85 |
| Восток (X>9.55) | Зазор 0.1м, затем западная стена техпомещения (`Maint_Inner_West_North/South`, X=9.65..9.85), дверь техпомещения на Z=-23.6..-22.4 | X: 9.55..11.55 |
| Юг/дверь (Z>-25.0) | Открытый пол главного коридора (`Floor_Main`/`Floor_NE`) | Z: -25.0..-23.0 |
| Север/тыл (Z<-30.0) | Снаружи здания (`BUILDING_LENGTH_Z` кончается на Z=-30) — за южной стеной шахты сразу внешняя граница уровня | Z: -30.0..-32.0 |

### Карта сверху (1 символ ≈ 0.3м, X слева-направо, Z сверху-вниз)
...
-30.8                                                           
-30.5                                                           
-30.2  N N N N N N # # # # # # # # # # # # # # # # # M M M M M M   <- граница Z=-30 (снаружи/внутри пополам в этой ячейке)
-29.9  N N N N N N # # E E E E E E E E E E E E E # # M M M M M M
 ...   (кабина лифта "E", борта "#", без изменений до Z=-25.4)
-25.4  N N N N N N # # # # # # D D D D D # # # # # # M M M M M M   <- North Wall, дверной проём "D"
-25.1  . . . . . . # # # # # # D D D D D # # # # # # M M M M M M
-24.8  . . . . . . . . . . . . . . . . . . . . . . # M M M M M M   <- открытый коридор, справа стена техпомещения
-23.9  . . . . . . . . . . . . . . . . . . . . . . D M M M M M M   <- дверь техпомещения (Z=-23.6..-22.4)
```
