...
> Сеттинг, сюжет и повествовательная структура вынесены в отдельный документ — **[LORE.md](LORE.md)**. Здесь остаётся только игровой цикл (пейсинг), напрямую влияющий на технические решения ниже.
...
> Нарративный дизайн (кассеты, правила подачи сюжета) и структура уровня по сюжетным точкам — см. [LORE.md](LORE.md).

---

## 2. 🏗️ Архитектура Проекта в Godot 4.7.2

> **Примечание:** разделы 2.1–2.2 описывают изначально спланированную структуру проекта
> (до реализации). Часть узлов и путей с тех пор изменилась — например, вместо одной
> `hotel_level.tscn` есть 10 сцен `hotel_level_1..10.tscn`, вся геометрия строится процедурно
> в рантайме через `hotel_level_generator.gd`, а вместо единственной `ExitDoor` теперь дверь
> стоит в каждом проёме (`entities/props/door.tscn`). Актуальная структура и правила —
> в **[.agents/AGENTS.md](.agents/AGENTS.md)**.
...
> **Фактическая реализация (2026-08-22) отличается от изначального плана ниже**: все 10 этажей
> отеля физически сосуществуют в одной сцене одновременно (см. `_generate_level()` в
> `hotel_level_generator.gd`), поэтому освещение — **не BAKED**, а обычные динамические
> `OmniLight3D`/`SpotLight3D`, которые генератор включает только на этаже, где сейчас находится
> игрок, и выключает на всех остальных (`_set_lit_floor()`, вызывается из `_process()` по Y-позиции
> игрока). LightmapGI для статичного baked-освещения не используется, так как оно не может быть
> включено/выключено поэтажно во время игры.

```
Изначальный план (не используется):
...
2. interact() → open_angle = -PI/2  (дверь открывается в коридор, от игрока)
...
> **2026-08-22:** таблица ниже приведена к реальным значениям из `hotel_level_generator.gd`
> (константы `BUILDING_WIDTH_X`, `DOUBLE_ROOM_BASE_X` и т.д., собранные в один блок в рамках
> оптимизации кода под ИИ-агентов) — более ранняя версия описывала числа из первоначального
> дизайна, которые разошлись с фактическим генератором. Авторитетный источник по геометрии —
> **[.agents/AGENTS.md](.agents/AGENTS.md)**, эта таблица — только верхнеуровневая сводка.

Следующие значения **не должны изменяться** без обновления всей системы генерации:

| Инвариант | Значение | Константа |
|-----------|----------|------|
| Игровой пол Y | `0.0` | все сцены |
| Ширина здания | `25.3 м` (X: ±12.65) | `BUILDING_WIDTH_X` |
| Коридор (внутренние грани) | X: `-2.75` … `4.85` | `CORRIDOR_WEST_EDGE_X` / `CORRIDOR_EAST_EDGE_X` |
| Origin двойных номеров X | `-7.65` | `DOUBLE_ROOM_BASE_X` |
| Origin одиночных номеров X | `+8.7` | `SINGLE_ROOM_BASE_X` |
| Высота этажа (пол-пол) | `4.5 м` | `BASE_FLOOR_TO_FLOOR_HEIGHT` |
| Толщина перекрытия | `0.5 м` | `BASE_FLOOR_THICKNESS` |
| Южная лестница, зона | Z: `25.0` … `30.0` | `SOUTH_STAIRS_ZONE_Z_START/END` |
| `basis.z` двери | **всегда в коридор** | `door.tscn` + логика `door.gd` |

Расположение мебели по комнатам (кровати, шкаф, стол) больше не захардкожено —
`_generate_double_room`/`_generate_single_room` читают его из словарей `DOUBLE_ROOM_LAYOUT` /
`SINGLE_ROOM_LAYOUT` в том же файле.
...
- [ ] Разместить точки взаимодействия (VhsTape × 3, ExitDoor на 4 этаже)
...
- [ ] Настроить поток событий: Кассета #3 у лифтов → выход → Цербер
